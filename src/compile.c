...
u8 *machine_compile(machine_t *m, str_t source) {
    ...
    i64 text_idx = 0, symtab_idx = 0, rela_idx = 0, rodata_idx = 0;
    {
        ...
        for (i64 idx = 0; idx < ehdr->e_shnum; idx++) {
            ...
            char *str = (char *)(elfbuf + shstr_shdr->s_offset + shdr->s_name);
    ...
    if (rela_idx == 0 || rodata_idx == 0) {
        return cache_add(m->cache, m->state.pc, elfbuf + text_shdr->s_offset,
                         text_shdr->s_size, text_shdr->s_addralign);
    }
    ...
    u64 text_addr = 0;
    {
        ...
        elf64_shdr_t *shdr = (elf64_shdr_t *)(elfbuf + shoff);
        cache_add(m->cache, m->state.pc, elfbuf + shdr->s_offset,
                  shdr->s_size, shdr->s_addralign);
        text_addr = (u64)cache_add(m->cache, m->state.pc, elfbuf + text_shdr->s_offset,
                                   text_shdr->s_size, text_shdr->s_addralign);
    }
    ...
    // apply relocations to .text section.
    {
        ...
        elf64_shdr_t *shdr = (elf64_shdr_t *)(elfbuf + shoff);
        i64 rels = shdr->s_size / sizeof(elf64_rela_t);
...
#endif
            elf64_rela_t *rel = (elf64_rela_t *)(elfbuf + shdr->s_offset + idx * sizeof(elf64_rela_t));
            assert(rel->r_type == R_X86_64_PC32);

            elf64_sym_t *sym = (elf64_sym_t *)(elfbuf + symtab_shdr->s_offset + rel->r_sym * sizeof(elf64_sym_t));
